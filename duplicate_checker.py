    ...
    try:
        ...
        for item_id, count in ids.items():
            if count > 1:
                ...
                dubl_lines.append(Fore.RED + f'[!] DUPLICATE!  Item ID: {item_id}  (count: {count})' + Style.RESET_ALL)
        if not found_duplicates:
            dubl_lines.append(Fore.GREEN + '[OK] No duplicate item IDs found!' + Style.RESET_ALL)
        ...
        print_frame('ERROR', [Fore.RED + f'[X] File "{file_path}" not found!' + Style.RESET_ALL], color=Fore.RED)
        return False
    except Exception as e:
        print_frame('ERROR', [Fore.RED + f'[X] Error: {e}' + Style.RESET_ALL], color=Fore.RED)
        return False
...
def main():
    ...
    if has_duplicates:
        print(Fore.RED + '\n[!] DUPLICATE ITEM IDs FOUND! Please resolve them.' + Style.RESET_ALL)
    else:
        print(Fore.GREEN + '\n[OK] All item IDs are unique!' + Style.RESET_ALL)
